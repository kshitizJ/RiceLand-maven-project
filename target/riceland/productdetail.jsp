   ...
   <%@page import="detail.Product" %>
      <%@page import="dao.ProductDao" %>
         <%@page import="db.DbConnect" %>
            <%@page import="detail.Review" %>
               <%@page import="java.util.List" %>
                  <%@page import="dao.ReviewDao" %>
                     <%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
                        <% response.setHeader("Cache-Control", "no-cache, no-store, must-revalidate" ); User
                           user1=(User) session.getAttribute("user"); if (user1 !=null) { %>
                           <!DOCTYPE html>
                           <html>
                           ...
                           <body>
                              <%@include file="./all_components/navbar.jsp" %>
                                 <% int id=Integer.parseInt(request.getParameter("id")); ProductDao productdao=new
                                    ProductDao(DbConnect.getCon()); Product product=productdao.getProduct(id); UserDao
                                    userdao=new UserDao(DbConnect.getCon()); ReviewDao reviewdao=new
                                    ReviewDao(DbConnect.getCon()); List<Review> reviews = reviewdao.getReview(id);
                                    %>
                                             ...
                                             <div class="col-md-5 col-12 text-center">
                                                <p class="breadcrumb">
                                                   <a href="./index.jsp">Home</a>/<a
                                                      href="./products.jsp">Products</a>/Details
                                                </p>
                                                <h1>
                                                   <%=product.getName()%>
                                                </h1>
                                                ...
                                                <a href="AddToCart?pid=<%=product.getPid()%>&uid=<%=user.getId()%>&pname=<%=product.getName()%>&price=<%=product.getPrice()%>"
